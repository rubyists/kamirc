module KamIRC
  ...
  REPLIES = {
    ...
    (ERR_NOSUCHSERVER = 402) => "%{server_name} :No such server",
    # Used to indicate the given channel name is invalid.
    (ERR_NOSUCHCHANNEL = 403) => "%{channel_name} :No such channel",
    ...
    (ERR_CANNOTSENDTOCHAN = 404) => "%{channel_name} :Cannot send to channel",
    # Sent to a user when they have joined the maximum number of allowed channels and they try to join another channel.
    (ERR_TOOMANYCHANNELS = 405) => "%{channel_name} :You have joined too many channels",
    ...
    (ERR_FILEERROR = 424) => ":File error doing %{file_op} on %{file}",
    ...
    (RPL_AWAY = 301) => "%{nick} :%{away_message}",
    ...
    (RPL_REHASHING = 382) => "%{config_file} :Rehashing",
    ...
    (RPL_TIME = 391) => "%{server} :%{string_showing_server's_local_time}",
    ...
    (RPL_TRACELINK = 200) => "Link %{version_&_debug_level} %{destination} %{next_server}",
    (RPL_TRACECONNECTING = 201) => "Try. %{class} %{server}",
    (RPL_TRACEHANDSHAKE = 202) => "H.S. %{class} %{server}",
    (RPL_TRACEUNKNOWN = 203) => "???? %{class} [%{client_IP_address_in_dot_form}]",
    ...
    (RPL_TRACENEWTYPE = 208) => "%{newtype} 0 %{client_name}",
    ...
    (RPL_TRACELOG = 261) => "File %{logfile} %{debug_level}",
    (RPL_STATSLINKINFO = 211) => "%{linkname} %{sendq} %{sent_messages} %{sent_bytes} %{received_messages} %{received_bytes} %{time_open}",
    ...
    (RPL_STATSYLINE = 218) => "Y %{class} %{ping_frequency} %{connect_frequency} %{max_sendq}",
    (RPL_ENDOFSTATS = 219) => "%{stats_letter} :End of /STATS report",
    ...
    (RPL_UMODEIS = 221) => "%{user_mode_string}",
    (RPL_LUSERCLIENT = 251) => ":There are %{users} users and %{invisible} invisible on %{servers} servers",
    (RPL_LUSEROP = 252) => "%{operators} :operator(s) online",
    (RPL_LUSERUNKNOWN = 253) => "%{unknown} :unknown connection(s)",
    ...
    (RPL_ADMINLOC1 = 257) => ":%{admin_info}",
    (RPL_ADMINLOC2 = 258) => ":%{admin_info}",
